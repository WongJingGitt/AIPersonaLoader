...
- [ ] **跨设备数据同步**：研究使用 `chrome.storage.sync` 实现人设数据在多设备间的自动同步。
- [ ] **Token 智能感知**：在注入时估算 Token 长度，并在超长时给出提示或进行智能截断。
- [ ] **高级变量支持**：在人设中支持如 `{{CLIPBOARD}}` 等动态变量，注入时自动替换为剪贴板内容。
- [ ] **社区分享**：提供一键分享/导入人设方案的功能，方便用户交流优秀的人设配置。